...
from src.models.customDateEntry import CustomDateEntry
...
class AddTaskView(tk.Toplevel):
    ...
    def create_datepicker(self, text, row, entry_font, label_font):
        ...
        # date_entry = DateEntry(self, width=38, font=entry_font, background="darkblue", foreground="white", borderwidth=1)
        date_entry = CustomDateEntry(self, width=38, font=entry_font,
    ...
    def submit_task(self):
        """提交任务，验证并调用回调"""
        title = self.title_entry.get().strip()  # 移除首尾空格
        description = self.description_entry.get("1.0", "end-1c").strip()  # 描述允许为空
        ...
        if start_date is None:
            messagebox.showwarning("Input Error", "Please set a Start Date for the task.")
            return

        if end_date is None:
            messagebox.showwarning("Input Error", "Please set an End Date for the task.")
            return
        # fix end

        # fix start (error-2)
        if end_date < start_date:
            messagebox.showwarning("Input Error", "End Date cannot be earlier than Start Date.")
            return
        # fix end
